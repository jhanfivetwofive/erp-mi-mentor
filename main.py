...
def _handle_any_exc(e):
    ...
    if request.args.get("debug") == "1":
        ...
        return Response(
            f"<h3>Excepción:</h3><pre>{tb}</pre>", status=500, mimetype="text/html"
        )
    try:
...
PREGUNTAS_DEF = {
    "R1": {
        "texto": "¿Tienes actualmente alguna fuente de ingreso activa?",
        "opciones": [
            ("3", "Sí, ingresos estables mensuales."),
            ("2", "Sí, pero son variables o inestables."),
            ("1", "No tengo ingresos actualmente."),
        ],
    },
    "R2": {
        "texto": "¿De cuanto son tus ingresos mensuales aproximadamente?",
        "opciones": [
            ("3", "Más de $30,000 MXN."),
    ...
    "R3": {
        "texto": "¿Tienes personas que dependan de ti?",
        "opciones": [
            ("3", "Sí, y compartimos ingresos/responsabilidades."),
            ("2", "Mi familia cercana depende de mí."),
            ("1", "Aparte de mi familia cercana más personas dependen de mí."),
        ],
    },
    "R4": {
        "texto": "¿Tu numero de libertad financiera es más, menos o igual de tus ingresos actuales ?",
        "opciones": [
            ("3", "Mucho menos de lo que ingreso mensualmente"),
            ("2", "Igual a lo que ingreso mensualmente"),
            ("1", "Mucho más de lo que ingreso mensualmente"),
        ],
    },
    "R5": {
        "texto": "¿Cuánto puedes invertir mensualmente sin afectar tus gastos básicos?",
        "opciones": [
            ("3", "Más de $5,000 MXN."),
    ...
    "R6": {
        "texto": "¿Tienes algún ahorro o capital disponible para invertir? ¿y cual sería el monto aproximado?",
        "opciones": [
            ("3", "Más de $50,000 MXN."),
    ...
    "R7": {
        "texto": "¿Tienes acceso a financiamiento?",
        "opciones": [
            ("3", "Sí, y lo ha usado estratégicamente."),
            ("2", "Sí, pero aún no lo usa."),
            ("1", "No tiene acceso a crédito o lo usa mal."),
        ],
    },
    "R8": {
        "texto": "¿Tienes alguna deuda activa actualmente?",
        "opciones": [
            ("3", "No tengo deudas"),
    ...
    "R9": {
        "texto": "¿Qué tan dispuesto estás a seguir un plan de acción con la guía de mentores?",
        "opciones": [
            ("3", "Totalmente comprometido y abierto a cambios."),
            ("2", "Interesado pero con algunas dudas."),
            ("1", "No está seguro o le cuesta seguir procesos."),
        ],
    },
    "R10": {
        "texto": "Si hoy tuvieras una estrategia clara para invertir, ¿te comprometerías a ejecutarla?",
        "opciones": [
            ("3", "Sí, inmediatamente."),
    ...
    "R11": {
        "texto": "En una escala del 1 al 10, ¿qué tan importante es para ti lograr la libertad financiera?",
        "opciones": [
            ("3", "9-10: Es su máxima prioridad."),
...
def _postventa_kpis():
    ...
    try:
        ...
        return {
            k: int(row[k] or 0)
            for k in ("total", "ventas", "viables", "requiere_ajuste", "no_viable")
        }
    except Exception:
        return {
            "total": 0,
...
def _postventa_insights_data(date_from=None, date_to=None, generacion=None):
    ...
    kpis = {
        ...
        "requiere_ajuste": (
            int(row["requiere_ajuste"])
            if row and row["requiere_ajuste"] is not None
            else 0
        ),
        "no_viable": (
    ...
    for r in client.query(
        q_series, job_config=bigquery.QueryJobConfig(query_parameters=params)
    ).result():
    ...
    for r in client.query(
        q_viab, job_config=bigquery.QueryJobConfig(query_parameters=params)
    ).result():
    ...
    for r in client.query(
        q_qs, job_config=bigquery.QueryJobConfig(query_parameters=params)
    ).result():
    ...
    for r in client.query(
        q_conv, job_config=bigquery.QueryJobConfig(query_parameters=params)
    ).result():
    ...
    gen_rows = []
    for r in client.query(
        q_gens, job_config=bigquery.QueryJobConfig(query_parameters=params)
    ).result():
        gen_rows.append(
            {
                "g": r["g"],
    ...
    return {
        "kpis": kpis,
        "by_date_labels": by_date_labels,
        "by_date_counts": by_date_counts,
        "viability_labels": viability_labels,
...
def _resolve_gid(any_id_or_label: str) -> dict | None:
    ...
    FROM `fivetwofive-20.INSUMOS.VW_ADQ_GENERACION_VENTANA`
    WHERE gid = @s OR label = @s
    LIMIT 1
    """
    job = bigquery.QueryJobConfig(
...
def _gen_meta(gid: str) -> dict | None:
    ...
    WHERE UPPER(TRIM(ID_GENERACION_PROGRAMA)) = @gid
    LIMIT 1
    """
    ...
    # 2) Fallback con DV (por si el catálogo no tiene la fila o fechas vacías)
    q_dv = """
    SELECT
        UPPER(TRIM(ID_GENERACION_PROGRAMA)) AS gid,
        UPPER(TRIM(MAX(GENERACION_PROGRAMA))) AS label,  -- ej. INVERSIONES INFINITAS G - 01
...
def _adq_insights_data(
    generacion=None, generacion_num=None, date_from=None, date_to=None
):
    """
    ...
    if gen_raw:  # ===== CAMINO 1: por GENERACION (usar sábana) =====
        ...
        """
        row = next(
            iter(client.query(q_met, job_config=job).result()), None
        )  # FIX: obtener row ANTES de usarlo
        if not row:
            return {
                "kpis": {
                    "leads": 0,
    ...
    try:
        ...
        if rol == "invitado" and request.method in READ_ONLY_BLOCKED_METHODS:
            ...
            try:
                return (
                    render_template(
                        "no_autorizado.html", msg="Modo solo lectura (invitado)"
                    ),
                    403,
                )
            except Exception:
...
def login_firebase():
    try:
        ...
        if not email_allowed(email):
            ...
            return (
                jsonify(
                    {"error": "Solo cuentas @fivetwofive.mx pueden iniciar sesión."}
                ),
                403,
            )
...
def alumnos_page():
    ...
    try:
        ...
        if request.args.get("debug") == "1":
            ...
            return Response(
                f"<pre>{traceback.format_exc()}</pre>", status=500, mimetype="text/html"
            )
        raise
    ...
    try:
        ...
        try:
            ...
            job_diag = bigquery.QueryJobConfig(
                query_parameters=[
                    bigquery.ScalarQueryParameter("c", "STRING", correo_norm)
                ]
            )
        ...
        if e164:
            alumno_nombre = (
                alumno_info.get("NOMBRE_ALUMNO")
                if isinstance(alumno_info, dict)
                else ""
            ) or ""
    ...
    try:
        ...
        if estado not in ESTADOS_PERMITIDOS:
            return (
                jsonify(
                    {
                        "error": f"Estado inválido. Usa: {', '.join(sorted(ESTADOS_PERMITIDOS))}"
                    }
                ),
                400,
            )
...
def postventa_insights():
    ...
    try:
        return render_template(
            ...
            kpis=data["kpis"],
            by_date_labels=data["by_date_labels"],
            by_date_counts=data["by_date_counts"],
            viability_labels=data["viability_labels"],
            viability_counts=data["viability_counts"],
            questions_labels=data["questions_labels"],
            questions_avg=data["questions_avg"],
            conv_labels=data["conv_labels"],
...
def postventa_agenda_nuevo_page():
    ...
    return render_template(
        "postventa_agenda_nuevo.html",
        default_fecha=hoy,
        default_hora=default_hora,
        ...
        if "calificacion" in data:
            try:
                newv["calificacion"] = (
                    ...
                    if data["calificacion"] not in ("", None)
                    else None
                )
...
def adquisicion_insights():
    ...
    g = (
        request.args.get("gen") or request.args.get("generacion") or ""
    ).strip() or None
    ...
    return render_template(
        "adquisicion_insights.html", gen=g or "", f_from=f or "", f_to=t or "", **data
    )
